...
[~,uStaticF1] = static_eq_shell_thermal(Assembly, F0, zeros(myMesh.nNodes,1), zeros(myMesh.nNodes,1), 'display', 'iter-detailed');
[~,uStaticF2] = static_eq_shell_thermal(Assembly, -F0, zeros(myMesh.nNodes,1), zeros(myMesh.nNodes,1), 'display', 'iter-detailed');
...
Vcold = orth([VMsCold{1,1},MDsCold{1,1},uStaticF1,uStaticF2,uStaticT]);
...
title('normalized global RMS error');
...
hold on;
semilogy(dynFull.nlin.time,100*errROMwDisp,'-k','linewidth',linewidth)
semilogy(dynFull.nlin.time,100*errROMmembrane,'-r','linewidth',linewidth)
semilogy(dynFull.nlin.time,100*errROMrotation,'-b','linewidth',linewidth)
legend('disp z','rotation','disp membrane');
xlabel('time [s]','fontsize', fontsize);
ylabel('error % ', 'fontsize', fontsize);
...
semilogy(dynFull.nlin.time,100*errMomentY ,'-r','linewidth',linewidth)
title('global normalized RMS error on internal moments');
xlabel('time [s]','fontsize', fontsize);
ylabel('error % ', 'fontsize', fontsize);
ax = gca; grid on; ax.FontSize = fontsize;
legend('Mx','My');
...
for ii = 1:length(tPlot)
    ...
    subplot 121
    PlotFieldonMesh(nodes,elementsPlot,squeeze(bendingMomentFull(:,1,ii)));
    title('HFM')
    caxis([-3, 3]); 
    txt = text('Units', 'Normalized', 'Position', [0.5, 0.9], 'string',...
    ['t: ',num2str(round(tii,5)),'s'], 'fontsize', 12); drawnow %limitrate
    ...
    subplot 122
    PlotFieldonMesh(nodes,elementsPlot,squeeze(bendingMomentRed(:,1,ii)));
    title('RED')
    caxis([-3, 3]); 
end
sgtitle('comparison full and Red Model bending Moment M_x')
