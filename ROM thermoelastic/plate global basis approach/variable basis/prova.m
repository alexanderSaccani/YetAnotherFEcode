...
% ROM (Cold Basis)
V = Assembly.unconstrain_vector(Vc);
constr_dofs = Assembly.Mesh.EBC.constrainedDOFs(:,1);
% V = eye(nDofsFree);
% V(constr_dofs,constr_dofs) = zeros(length(constr_dofs),length(constr_dofs));
% V(:,constr_dofs) = [];
...
TI_NL_r = ImplicitNewmarkRed('timestep',h,'alpha',0.005);

%set function for chosing the basis
%Vfun_p(p), Vfun_t(t), Vfun_t = @(t) Vfun_p(pfun(t))
Vfun = @(t) V;

% nonlinear Residual evaluation function handle
residual_r = @(q,qd,qdd,t,Vfun)residual_thermal_VB(q,qd,qdd,t,Assembly,Vfun,Fext,Tdynt,gradTt);

% integrate equations with Newmark scheme
TI_NL_r.Integrate(q0,qd0,qdd0,tint,residual_r,Vfun);
...
dynRedCold.nlin.vel = decodeDofsNodes(vDynRed,nNodes,nDofPerNode); % (node, dof of node, tsamp)
...
%% Integrate nonlinear ROM Old Method (cold basis)
% ROM (Cold Basis)
V = Vc;
...
%save solution
dynRedColdOld.nlin.disp = decodeDofsNodes(uDynRed,nNodes,nDofPerNode); % (node, dof of node, tsamp)
dynRedColdOld.nlin.time = TI_NL_r.Solution.time;
dynRedColdOld.nlin.vel = decodeDofsNodes(vDynRed,nNodes,nDofPerNode); % (node, dof of node, tsamp)
% save('saved_simulations/ROMColdColdPlate.mat','dynRedCold');
...
plot(dynRedColdOld.nlin.time,squeeze(dynRedColdOld.nlin.disp(node2plot,dofPl,:)),'--r','linewidth',linewidth)
plot(dynRedCold.nlin.time,squeeze(dynRedCold.nlin.disp(node2plot,dofPl,:)),'-b','linewidth',linewidth)
...
ax = gca; grid on; ax.FontSize = fontsize; legend('old','new','fontsize',fontsize);


figure('units','normalized','position',[0.3,0.1,0.6,0.7]); hold on
plot(dynRedColdOld.nlin.time,squeeze(dynRedColdOld.nlin.vel(node2plot,dofPl,:)),'--r','linewidth',linewidth)
plot(dynRedCold.nlin.time,squeeze(dynRedCold.nlin.vel(node2plot,dofPl,:)),'-b','linewidth',linewidth)
xlabel('time [s]','fontsize',fontsize); ylabel('u [m]','fontsize', fontsize)
title(['vel, normalized plotted dof location: ',num2str(plotDofLocation(1)),',',...
    num2str(plotDofLocation(2)),',  dof: ',num2str(dofPl)]);
ax = gca; grid on; ax.FontSize = fontsize; legend('old','new','fontsize',fontsize);
