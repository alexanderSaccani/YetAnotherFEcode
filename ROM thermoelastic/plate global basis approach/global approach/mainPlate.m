...
%% Create a Global bais (stacking approach)
sizeSingleROM = size(ROMs{1}.V,2);
nDOFsFull = size(ROMs{1}.V,1);
Vall = zeros(nDOFsFull,sizeSingleROM*length(p));
Vall = flip(Vall,2);

for ii = 1:length(p) 
    ind = (ii-1)*sizeSingleROM + 1;
    Vall(:,ind:ind+sizeSingleROM-1) = ROMs{ii}.V;
end

thetaThreshold = 0.5;
Vstack = Vc;
jj = [];
for ii = 1:size(Vall,2)
    
    angle_ii = 180/pi*subspace(Vstack,Vall(:,ii));
    if angle_ii > thetaThreshold
         Vstack = [Vstack,Vall(:,ii)];
         jj = [jj,ii];
    end
...
disp(['Global basis has size ',num2str(size(Vstack,2)),...
    ' with maximum allowed angle of ',num2str(thetaThreshold),' deg between subspaces'])
...
%plot HFM simulation
dof2plot = 3;
plot_dof_location = [0.2,0.2]; %percentage of length of beam
...
%save('saved_simulations_new/ROMCold90.mat','dynRedCold');

else
filenameROMRun = 'saved_simulations_new/ROMCold90.mat';
load(filenameROMRun)
end
...
%% Integrate nonlinear ROM (global basis)
V = Vstack;
...
%save solution
dynRedGlobal.nlin.disp = decodeDofsNodes(uDynRed,nNodes,nDofPerNode); % (node, dof of node, tsamp)
dynRedGlobal.nlin.time = TI_NL_r.Solution.time;

%save('saved_simulations_new/GlobalBasisRun.mat','dynRedGlobal');

else
    
filenameROMRunGlobal = 'saved_simulations_new/GlobalBasisRun.mat';
load(filenameROMRunGlobal)
...
title(['normalized plotted dof location: ',num2str(plotDofLocation(1)),',',...
    num2str(plotDofLocation(2)),',  dof: ',num2str(dofPl)]);
ax = gca; grid on; ax.FontSize = fontsize; legend('HFM','RED cold','RED global','fontsize',fontsize);
...
%RMS error
errROM = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedCold.nlin.disp,dynRedCold.nlin.time,gdlErr,nodesErr);
errROMrotation = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedCold.nlin.disp,dynRedCold.nlin.time,4:5,nodesErr);
errROMwDisp = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedCold.nlin.disp,dynRedCold.nlin.time,3,nodesErr);
errROMmembrane = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedCold.nlin.disp,dynRedCold.nlin.time,1:2,nodesErr);

errROMGlobal = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedGlobal.nlin.disp,dynRedGlobal.nlin.time,gdlErr,nodesErr);
errROMrotationGlobal = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedGlobal.nlin.disp,dynRedGlobal.nlin.time,4:5,nodesErr);
errROMwDispGlobal = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedGlobal.nlin.disp,dynRedGlobal.nlin.time,3,nodesErr);
errROMmembraneGlobal = RMS_error_ROM(dynFull.nlin.disp,dynFull.nlin.time,dynRedGlobal.nlin.disp,dynRedGlobal.nlin.time,1:2,nodesErr);

%plot error
figure('units','normalized','position',[0.3,0.1,0.5,0.5]); 
hold on;
semilogy(dynFull.nlin.time,100*errROMwDisp,'-r','linewidth',linewidth)
semilogy(dynFull.nlin.time,100*errROMmembrane,'-b','linewidth',linewidth)
semilogy(dynFull.nlin.time,100*errROMrotation,'-k','linewidth',linewidth)

xlabel('time [s]','fontsize', fontsize);
ylabel('error % ', 'fontsize', fontsize);
title('global normalized RMS error');
ax = gca; grid on; ax.FontSize = fontsize;

hold on 
semilogy(dynFull.nlin.time,100*errROMwDispGlobal,'--r','linewidth',linewidth)
semilogy(dynFull.nlin.time,100*errROMmembraneGlobal,'--b','linewidth',linewidth)
semilogy(dynFull.nlin.time,100*errROMrotationGlobal,'--k','linewidth',linewidth)

legend('disp z cold','rotation cold','disp membrane cold','disp z global',...
    'rotation global','disp membrane global');

%full solution animation
eq = sampledEqDisp{5};
eq = reshape(eq,5,[]).';
figure
PlotFieldonDeformedMesh([nodes,zNodes],elementsPlot, [eq(:,1),...
...
% dispFull = dynFull.nlin.disp;
% dispFullx = squeeze(dispFull(:,1,:));
% dispFully = squeeze(dispFull(:,2,:));
% dispFullz = squeeze(dispFull(:,3,:));
% nNodes = Assembly.Mesh.nNodes;
% dispForAnimation = zeros(nNodes*3,size(dispFullx,2));
% dispForAnimation(1:3:end,:) = dispFullx;
% dispForAnimation(2:3:end,:) = dispFully;
% dispForAnimation(3:3:end,:) = dispFullz;
% figure
% AnimateFieldonDeformedMesh([nodes,zNodes],elementPlot,dispForAnimation,'framerate',round(frameRate),'factor',10)
