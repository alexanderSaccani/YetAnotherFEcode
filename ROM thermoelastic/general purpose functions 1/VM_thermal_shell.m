function [VM,static_eq,omega] = VM_thermal_shell(assembly,T_samples,gradT_samples,eq,n_VMs)
...
for ii = 1:n_configs
    ...
    if eq == 0
        ...
        [K_ii,~] = assembly.tangent_stiffness_and_force(zeros(nDofsF,1),T_ii,gradT_ii); %optimizable
        assembly.DATA.K = K_ii;    % this value of K is used only for first guess solution (T not considered in the linear guess!)
        [~,eq_ii] = static_eq_shell_thermal(assembly, F, T_ii,gradT_ii); %not super efficient since it computes also the linear solution
    end

    static_eq(:,ii) = eq_ii; %static equilibrium

    %compute tangent stiffness matrix at static equilibrium (linearized model)
    [K_ii,~] = assembly.tangent_stiffness_and_force(eq_ii,T_ii,gradT_ii);
