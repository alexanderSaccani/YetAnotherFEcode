...
%     (default value is 0).
%   - varargTanStiff : cell array in which the additional arguments (to
%    displacements) used to compute the tangent stiffness matrix are stored.
%    Pay attention to order of elements. For example we could have
%    varargTanStiff = {T} for continuum thermal and varargTanStiff =
%    {T,gradT} for shell thermal
%   - varargStiffDer: cell array in which the values of 
%   - d_eq : free vector of equilibrium displacements, around which the
%   VM are expanded.
%
% OUTPUTS
...
function [MD, names] = modal_derivatives_thermal(myAssembly, elements, d_eq, Phi, varargTanStiff, varargTanStiffDer, USEJULIA)
...
[K0,~] = myAssembly.tangent_stiffness_and_force(d_eq,varargTanStiff{:});
...
for jj = 1 : n_VMs
    ...
    if USEJULIA == 1
        ...
        dK_deta_j = myAssembly.matrix('stiffness_derivative',d_eq, Phi_j, varargTanStiffDer{:});
        fprintf(' %.2f s\n',toc(t0))
    end
