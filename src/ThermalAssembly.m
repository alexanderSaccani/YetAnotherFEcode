classdef ThermalAssembly < handle
    properties
    ...
    methods
        function self = ThermalAssembly(Mesh,varargin)
            self.Mesh = Mesh;   
            if nargin == 2
...
%         end
        ...
        function K =  conductivity_matrix(self)
             boundary = false;
             K = self.matrix('conductivity_matrix',boundary);
        end
        
        function M =  thermal_capacity_matrix(self)
            boundary = false;
            M = self.matrix('thermal_capacity_matrix',boundary);
        end
        
        function LM =  load_matrix(self)
            boundary = false;
            LM = self.matrix('load_matrix',boundary);
        end
        
        function TFM =  thermal_flux_matrix(self)
            boundary = true;
            TFM = self.matrix('thermal_flux_matrix',boundary);
        end
        

        function [K] = matrix(self,elementMethodName,boundary,varargin)
            
            % This function assembles a generic finite element matrix from
            % its element level counterpart. 'elementMethodName' is a
            % string input containing the name of the method (queried
            % method) that returns the element level matrix Ke. 'boundary'
            % is a boolean value that tells the code if the required matrix
            % is a method defined on  boundary elements (boundary == true)
            % or on inner elements (boundary == false).If boundary == true
            % then the queried method (stored as a string in
            % elementMethodName) must be defined on all boundary elements
            % in the assembly. If boundary == false then the queried method
            % must be defined in all non boundary elements in the assembly.
            ...
            %pick only boundary or not boundary elements
            LogicBoundaryElements = [self.Mesh.Elements(elementSet).isBoundary];            
            
            if boundary
                elementSet = elementSet(LogicBoundaryElements);
                %elementWeights = elementWeights(LogicBoundaryElements);
            else
                elementSet = elementSet(~LogicBoundaryElements);
                %elementWeights = elementWeights(~LogicBoundaryElements);
            end
